...
import net.inklinggamer.celestium.item.CelestiumShovelHelper;
...
public final class CelestiumShovelClient {
    ...
    public static boolean handleRightClickToggle(MinecraftClient client) {
        ...
        areaToggleHeld = true;
        CelestiumClientNetworking.send(new ToggleCelestiumShovelAreaModePayload());
        return true;
    }
    ...
    public static boolean handleGroundSlamAttempt(MinecraftClient client) {
        ...
        CelestiumClientNetworking.send(new ArmCelestiumShovelSlamPayload());
        return true;
    }
    ...
    private static void updateOutline(MinecraftClient client) {
        ...
        if (slamMiningSuppressed || !celestium$canUseAreaMining(client) || client.interactionManager == null) {
            return;
        }
        ...
        outlinePositions.addAll(celestium$getAreaMiningTargets(client, hitResult.getBlockPos(), hitResult.getSide()).outlinePositions());
    }

    private static void updateBreakingAnimation(MinecraftClient client) {
        ClientPlayerInteractionManager interactionManager = client.interactionManager;
        if (slamMiningSuppressed || !celestium$canUseAreaMining(client) || interactionManager == null) {
            clearBreakingAnimation(client);
            return;
        }
        ...
        List<BlockPos> targets = celestium$getAreaMiningTargets(client, currentBreakingPos, breakingFace).breakablePositions().stream()
    ...
    private static CelestiumShovelHelper.AreaMiningTargets celestium$getAreaMiningTargets(MinecraftClient client, BlockPos centerPos, Direction face) {
