...
public final class CelestiumSpearStunCooldownHud {
    ...
    public static void renderNearHotbar(DrawContext drawContext, PlayerEntity player) {
        ...
        drawContext.drawGuiTexture(BACKGROUND_TEXTURE, layout.x(), layout.y(), layout.width(), LeftHotbarStatusBarLayout.BAR_HEIGHT);

        int progressWidth = MathHelper.clamp((int) (getProgress() * layout.width()), 0, layout.width());
        if (progressWidth > 0) {
            drawContext.drawGuiTexture(PROGRESS_TEXTURE, layout.x(), layout.y(), progressWidth, LeftHotbarStatusBarLayout.BAR_HEIGHT);
        }
    }
