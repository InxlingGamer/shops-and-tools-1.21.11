...
public final class CelestiumShovelManager {
    ...
    private static final DustParticleEffect SLAM_PARTICLE = new DustParticleEffect(new org.joml.Vector3f(1.0F, 89.0F / 255.0F, 192.0F / 255.0F), 1.75F);
    ...
    private static void triggerGroundSlam(ServerPlayerEntity player, PlayerState state) {
        ...
        net.minecraft.registry.Registry<Structure> structureRegistry = world.getRegistryManager().get(RegistryKeys.STRUCTURE);
        RegistryEntryList<Structure> trialChambers = RegistryEntryList.of(
                structureRegistry.getEntry(structureRegistry.get(StructureKeys.TRIAL_CHAMBERS))
        );
        ...
        if (trialChamberPos == null) {
            player.sendMessage(Text.translatable("message.celestium.celestium_shovel_trial_chamber_not_found"), true);
            return;
        }
